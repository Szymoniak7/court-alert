export const CLUB_COLORS: Record<string, {
  hex: string;
  cell: string;
  badge: string;
  header: string;
}> = {
  'loba-padel':        {
    hex: '#34d399',
    cell: 'bg-emerald-500/20 border-emerald-500/35 text-emerald-200 hover:bg-emerald-500/30',
    badge: 'bg-emerald-950 text-emerald-300 border-emerald-800',
    header: 'text-emerald-400',
  },
  'mana-padel':        {
    hex: '#60a5fa',
    cell: 'bg-blue-500/20 border-blue-500/35 text-blue-200 hover:bg-blue-500/30',
    badge: 'bg-blue-950 text-blue-300 border-blue-800',
    header: 'text-blue-400',
  },
  'toro-padel':        {
    hex: '#fb923c',
    cell: 'bg-orange-500/20 border-orange-500/35 text-orange-200 hover:bg-orange-500/30',
    badge: 'bg-orange-950 text-orange-300 border-orange-800',
    header: 'text-orange-400',
  },
  'interpadel':        {
    hex: '#c084fc',
    cell: 'bg-purple-500/20 border-purple-500/35 text-purple-200 hover:bg-purple-500/30',
    badge: 'bg-purple-950 text-purple-300 border-purple-800',
    header: 'text-purple-400',
  },
  'warsaw-padel-club': {
    hex: '#fb7185',
    cell: 'bg-rose-500/20 border-rose-500/35 text-rose-200 hover:bg-rose-500/30',
    badge: 'bg-rose-950 text-rose-300 border-rose-800',
    header: 'text-rose-400',
  },
  'rqt-sport':         {
    hex: '#facc15',
    cell: 'bg-yellow-500/20 border-yellow-500/35 text-yellow-200 hover:bg-yellow-500/30',
    badge: 'bg-yellow-950 text-yellow-300 border-yellow-800',
    header: 'text-yellow-400',
  },
  'padlovnia':         {
    hex: '#22d3ee',
    cell: 'bg-cyan-500/20 border-cyan-500/35 text-cyan-200 hover:bg-cyan-500/30',
    badge: 'bg-cyan-950 text-cyan-300 border-cyan-800',
    header: 'text-cyan-400',
  },
  'rakiety-pge-narodowy': {
    hex: '#f87171',
    cell: 'bg-red-500/20 border-red-500/35 text-red-200 hover:bg-red-500/30',
    badge: 'bg-red-950 text-red-300 border-red-800',
    header: 'text-red-400',
  },
  'rakiety-aero': {
    hex: '#fca5a5',
    cell: 'bg-red-400/20 border-red-400/35 text-red-200 hover:bg-red-400/30',
    badge: 'bg-red-900 text-red-200 border-red-700',
    header: 'text-red-300',
  },
  'propadel': {
    hex: '#a78bfa',
    cell: 'bg-violet-500/20 border-violet-500/35 text-violet-200 hover:bg-violet-500/30',
    badge: 'bg-violet-950 text-violet-300 border-violet-800',
    header: 'text-violet-400',
  },
  'mera': {
    hex: '#2dd4bf',
    cell: 'bg-teal-500/20 border-teal-500/35 text-teal-200 hover:bg-teal-500/30',
    badge: 'bg-teal-950 text-teal-300 border-teal-800',
    header: 'text-teal-400',
  },
  'sporteum': {
    hex: '#a3e635',
    cell: 'bg-lime-500/20 border-lime-500/35 text-lime-200 hover:bg-lime-500/30',
    badge: 'bg-lime-950 text-lime-300 border-lime-800',
    header: 'text-lime-400',
  },
  'miedzeszyn': {
    hex: '#38bdf8',
    cell: 'bg-sky-500/20 border-sky-500/35 text-sky-200 hover:bg-sky-500/30',
    badge: 'bg-sky-950 text-sky-300 border-sky-800',
    header: 'text-sky-400',
  },
  'teniswil': {
    hex: '#f472b6',
    cell: 'bg-pink-500/20 border-pink-500/35 text-pink-200 hover:bg-pink-500/30',
    badge: 'bg-pink-950 text-pink-300 border-pink-800',
    header: 'text-pink-400',
  },
  'tenes': {
    hex: '#fbbf24',
    cell: 'bg-amber-500/20 border-amber-500/35 text-amber-200 hover:bg-amber-500/30',
    badge: 'bg-amber-950 text-amber-300 border-amber-800',
    header: 'text-amber-400',
  },

  // ── Śląsk ─────────────────────────────────────────────────────────────────
  'viva-padel-katowice': {
    hex: '#818cf8',
    cell: 'bg-indigo-500/20 border-indigo-500/35 text-indigo-200 hover:bg-indigo-500/30',
    badge: 'bg-indigo-950 text-indigo-300 border-indigo-800',
    header: 'text-indigo-400',
  },
  'padel-center-katowice': {
    hex: '#e879f9',
    cell: 'bg-fuchsia-500/20 border-fuchsia-500/35 text-fuchsia-200 hover:bg-fuchsia-500/30',
    badge: 'bg-fuchsia-950 text-fuchsia-300 border-fuchsia-800',
    header: 'text-fuchsia-400',
  },
  'padelup-katowice': {
    hex: '#86efac',
    cell: 'bg-green-400/20 border-green-400/35 text-green-200 hover:bg-green-400/30',
    badge: 'bg-green-950 text-green-300 border-green-800',
    header: 'text-green-400',
  },
  'padel-team-tychy': {
    hex: '#f97316',
    cell: 'bg-orange-600/20 border-orange-600/35 text-orange-200 hover:bg-orange-600/30',
    badge: 'bg-orange-950 text-orange-300 border-orange-800',
    header: 'text-orange-500',
  },
  'padel-team-bytom': {
    hex: '#fda4af',
    cell: 'bg-rose-300/20 border-rose-300/35 text-rose-200 hover:bg-rose-300/30',
    badge: 'bg-rose-900 text-rose-200 border-rose-700',
    header: 'text-rose-300',
  },
  'padelteam-zabrze': {
    hex: '#c4b5fd',
    cell: 'bg-violet-400/20 border-violet-400/35 text-violet-200 hover:bg-violet-400/30',
    badge: 'bg-violet-900 text-violet-200 border-violet-700',
    header: 'text-violet-300',
  },
  'padelmania-dabrowa': {
    hex: '#fde047',
    cell: 'bg-yellow-300/20 border-yellow-300/35 text-yellow-200 hover:bg-yellow-300/30',
    badge: 'bg-yellow-900 text-yellow-200 border-yellow-700',
    header: 'text-yellow-300',
  },
  'padel-on': {
    hex: '#6ee7b7',
    cell: 'bg-emerald-300/20 border-emerald-300/35 text-emerald-200 hover:bg-emerald-300/30',
    badge: 'bg-emerald-900 text-emerald-200 border-emerald-700',
    header: 'text-emerald-300',
  },
  'ultra-padel-gliwice': {
    hex: '#67e8f9',
    cell: 'bg-cyan-300/20 border-cyan-300/35 text-cyan-200 hover:bg-cyan-300/30',
    badge: 'bg-cyan-900 text-cyan-200 border-cyan-700',
    header: 'text-cyan-300',
  },
  'sport-park-slask': {
    hex: '#7dd3fc',
    cell: 'bg-sky-300/20 border-sky-300/35 text-sky-200 hover:bg-sky-300/30',
    badge: 'bg-sky-900 text-sky-200 border-sky-700',
    header: 'text-sky-300',
  },

  // ── Wrocław ───────────────────────────────────────────────────────────────
  'padel-pl-wroclaw': {
    hex: '#4ade80',
    cell: 'bg-green-500/20 border-green-500/35 text-green-200 hover:bg-green-500/30',
    badge: 'bg-green-950 text-green-300 border-green-800',
    header: 'text-green-400',
  },
  'fiesta-padel': {
    hex: '#f97316',
    cell: 'bg-orange-500/20 border-orange-500/35 text-orange-200 hover:bg-orange-500/30',
    badge: 'bg-orange-950 text-orange-300 border-orange-800',
    header: 'text-orange-400',
  },

  // ── Łódź ──────────────────────────────────────────────────────────────────
  'padel-lodz': {
    hex: '#d946ef',
    cell: 'bg-fuchsia-500/20 border-fuchsia-500/35 text-fuchsia-200 hover:bg-fuchsia-500/30',
    badge: 'bg-fuchsia-950 text-fuchsia-300 border-fuchsia-800',
    header: 'text-fuchsia-400',
  },
  'stacja-padel': {
    hex: '#86efac',
    cell: 'bg-green-300/20 border-green-300/35 text-green-100 hover:bg-green-300/30',
    badge: 'bg-green-900 text-green-200 border-green-700',
    header: 'text-green-300',
  },

  // ── Opole ─────────────────────────────────────────────────────────────────
  'pop-yard': {
    hex: '#93c5fd',
    cell: 'bg-blue-300/20 border-blue-300/35 text-blue-200 hover:bg-blue-300/30',
    badge: 'bg-blue-900 text-blue-200 border-blue-700',
    header: 'text-blue-300',
  },

  // ── Poznań ────────────────────────────────────────────────────────────────
  'interpadel-poznan': {
    hex: '#4f46e5',
    cell: 'bg-indigo-600/20 border-indigo-600/35 text-indigo-200 hover:bg-indigo-600/30',
    badge: 'bg-indigo-950 text-indigo-300 border-indigo-800',
    header: 'text-indigo-500',
  },
  'plek-poznan': {
    hex: '#65a30d',
    cell: 'bg-lime-600/20 border-lime-600/35 text-lime-200 hover:bg-lime-600/30',
    badge: 'bg-lime-950 text-lime-300 border-lime-800',
    header: 'text-lime-500',
  },
  'akademia-padla-bukowska': {
    hex: '#0e7490',
    cell: 'bg-cyan-700/20 border-cyan-700/35 text-cyan-200 hover:bg-cyan-700/30',
    badge: 'bg-cyan-950 text-cyan-300 border-cyan-800',
    header: 'text-cyan-600',
  },

  // ── Trójmiasto ────────────────────────────────────────────────────────────
  'interpadel-gdynia': {
    hex: '#0d9488',
    cell: 'bg-teal-600/20 border-teal-600/35 text-teal-200 hover:bg-teal-600/30',
    badge: 'bg-teal-950 text-teal-300 border-teal-800',
    header: 'text-teal-500',
  },
  'baltic-padel-club': {
    hex: '#dc2626',
    cell: 'bg-red-600/20 border-red-600/35 text-red-200 hover:bg-red-600/30',
    badge: 'bg-red-950 text-red-300 border-red-800',
    header: 'text-red-500',
  },
  'gdynia-padel-club': {
    hex: '#9333ea',
    cell: 'bg-purple-600/20 border-purple-600/35 text-purple-200 hover:bg-purple-600/30',
    badge: 'bg-purple-950 text-purple-300 border-purple-800',
    header: 'text-purple-500',
  },
  'padbox': {
    hex: '#d97706',
    cell: 'bg-amber-600/20 border-amber-600/35 text-amber-200 hover:bg-amber-600/30',
    badge: 'bg-amber-950 text-amber-300 border-amber-800',
    header: 'text-amber-500',
  },
  'padbox-kartuska': {
    hex: '#16a34a',
    cell: 'bg-green-600/20 border-green-600/35 text-green-200 hover:bg-green-600/30',
    badge: 'bg-green-950 text-green-300 border-green-800',
    header: 'text-green-500',
  },

  // ── Toruń ─────────────────────────────────────────────────────────────────
  'interpadel-torun': {
    hex: '#0284c7',
    cell: 'bg-sky-600/20 border-sky-600/35 text-sky-200 hover:bg-sky-600/30',
    badge: 'bg-sky-950 text-sky-300 border-sky-800',
    header: 'text-sky-500',
  },
  'city-padel-torun': {
    hex: '#f59e0b',
    cell: 'bg-amber-500/20 border-amber-500/35 text-amber-200 hover:bg-amber-500/30',
    badge: 'bg-amber-950 text-amber-300 border-amber-800',
    header: 'text-amber-400',
  },

  // ── Szczecin ──────────────────────────────────────────────────────────────
  'fabryka-energii': {
    hex: '#10b981',
    cell: 'bg-emerald-600/20 border-emerald-600/35 text-emerald-200 hover:bg-emerald-600/30',
    badge: 'bg-emerald-950 text-emerald-300 border-emerald-800',
    header: 'text-emerald-500',
  },

  // ── Bydgoszcz ─────────────────────────────────────────────────────────────
  'pura-padel': {
    hex: '#7c3aed',
    cell: 'bg-violet-600/20 border-violet-600/35 text-violet-200 hover:bg-violet-600/30',
    badge: 'bg-violet-950 text-violet-300 border-violet-800',
    header: 'text-violet-500',
  },
  'fast-tennis': {
    hex: '#0891b2',
    cell: 'bg-cyan-600/20 border-cyan-600/35 text-cyan-200 hover:bg-cyan-600/30',
    badge: 'bg-cyan-950 text-cyan-300 border-cyan-800',
    header: 'text-cyan-500',
  },

  // ── Bielsko-Biała ─────────────────────────────────────────────────────────
  'solar': {
    hex: '#eab308',
    cell: 'bg-yellow-500/20 border-yellow-500/35 text-yellow-200 hover:bg-yellow-500/30',
    badge: 'bg-yellow-950 text-yellow-300 border-yellow-800',
    header: 'text-yellow-400',
  },

  // ── Wrocław (cd.) ─────────────────────────────────────────────────────────
  'morskie-oko': {
    hex: '#059669',
    cell: 'bg-emerald-700/20 border-emerald-700/35 text-emerald-200 hover:bg-emerald-700/30',
    badge: 'bg-emerald-950 text-emerald-200 border-emerald-700',
    header: 'text-emerald-600',
  },

  // ── Pruszcz Gdański (Trójmiasto) ──────────────────────────────────────────
  'padel-park': {
    hex: '#be185d',
    cell: 'bg-pink-700/20 border-pink-700/35 text-pink-200 hover:bg-pink-700/30',
    badge: 'bg-pink-950 text-pink-300 border-pink-800',
    header: 'text-pink-600',
  },
};
